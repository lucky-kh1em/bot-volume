...
const sendTradeTx = async (amountIn, amountOutMin, paths, routerContract, account) => {
	...
	while (!isSwapSuccess) {
		try {
			console.log(
				`=========ACCOUNT ${account} SWAP ${parseInt(ethers.utils.formatEther(amountIn))}==========`
			);
...
const completeTrade = async (routerContract, tradeUndone) => {
	...
	const amountOutMinBusd = getAmountOutMin(ethers.utils.parseEther(amountInBusd.toString()));
	...
	tradeUndone.amountOutBusd = parseFloat(ethers.utils.formatEther(amountOutExactly));
...
const sellAndBuyBack = async (privateKey) => {
	try {
		...
		trade.amountOutBusd = parseFloat(ethers.utils.formatEther(amountOutBusdExactly));
...
const orderedTrade = async () => {
	try {
		for (let i = 0; i < config.privateKeyBots.length; i++) {
			sellAndBuyBack(config.privateKeyBots[i]);
			await sleep(180000);
		}
		setInterval(async () => {
			for (let i = 0; i < config.privateKeyBots.length; i++) {
